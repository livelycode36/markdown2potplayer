...
- 目前发现的bug：
  1. 全系urlencode的bug：如果路径中存在`\[`会让，在【ob的预览模式】下(回链会被ob自动urlencode)，`\`奇消失变为,`[`；例如：`G:\BaiduSyncdisk\123\[456]789.mp4` 在bug下变为：`G:\BaiduSyncdisk\123[456]789.mp4` <= 丢失了`\`，所以即使关闭编码也会强制在`\[`出现在路径中，将`\[`中的`\`进行编码。如果不想被编码，请不要这样给视频文件命名 或 使用`-`、`_`等替代
