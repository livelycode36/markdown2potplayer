...
test_error() {
    ...
    ; sock.Connect("www.google.com",80) ; 127.0.0.1",33660 ; www.google.com",80
...
test_server() { ; server - uses async to accept sockets
    ...
    sock.Bind("0.0.0.0",33660) ; "0.0.0.0",33660
...
test_client() { ; client
    ...
    sock.Connect("127.0.0.1",33660) ; 127.0.0.1",33660 ; www.google.com",80
...
test_send() { ; the connecting client doesn't use async
    sock := winsock("client", cb, "IPV4")
    If !(r := sock.Connect("127.0.0.1", 33660, true)) { ; connect as blocking, setting param #3 to TRUE
